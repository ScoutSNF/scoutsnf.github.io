import { CMS_METASTORE_ITEM_URL, CMS_DATASTORE_QUERY_URL } from './sources.js'
import { fetchWithRetry } from './fetchRetry.js'
import { parseCsvTable, type CsvTable } from './csv.js'
...
// CMS's datastore query API rejects `limit` values above some undocumented cap with a flat 400
// Bad Request rather than silently capping the page -- 500 is a conservative value well under
// any commonly-documented CKAN/DKAN datastore limit (matches src/data/dkan.ts).
...
async function fetchAllRowsViaQueryApi(datasetId: string, label: string, onRetry?: OnRetry): Promise<CsvTable> {
  ...
  while (true) {
    const res = await fetchWithRetry(`${CMS_DATASTORE_QUERY_URL(datasetId)}?limit=${pageSize}&offset=${offset}`, `${label} data`, undefined, {
      onRetry
    })
...
export async function fetchCmsDatasetTable(datasetId: string, label: string, onRetry?: OnRetry): Promise<CsvTable> {
  ...
  // The CSV distribution CMS attaches to a dataset's metadata isn't guaranteed to be the full,
  // current file -- sanity-check its row count against a single page of the live datastore query
  // API before trusting it (matches src/data/dkan.ts).
  try {
    const probeRes = await fetchWithRetry(`${CMS_DATASTORE_QUERY_URL(datasetId)}?limit=${QUERY_API_PAGE_SIZE}&offset=0`, `${label} data`, undefined, {
      onRetry
    })
