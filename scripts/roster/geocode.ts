import { CENSUS_GEOCODE_BATCH_URL, NOMINATIM_SEARCH_URL } from './sources.js'
...
function buildBatchCsv(inputs: GeocodeInput[]): string {
  return inputs.map((r) => [r.id, r.address, r.city, r.state, r.zip].map(csvField).join(',')).join('\n')
}
...
async function geocodeChunk(inputs: GeocodeInput[]): Promise<Map<string, GeocodeResult>> {
  ...
  // A 500-address batch can legitimately take close to 20s to match (matches src/data/geocode.ts).
  const res = await fetchWithRetry(CENSUS_GEOCODE_BATCH_URL, 'Census geocoder', { method: 'POST', body: form }, { timeoutMs: 60_000 })
...
export async function geocodeBatch(inputs: GeocodeInput[], onProgress?: (done: number, total: number) => void): Promise<Map<string, GeocodeResult>> {
...
export async function geocodeSingleNominatim(input: GeocodeInput): Promise<GeocodeResult | null> {
  ...
  // CI-only identifying param per Nominatim's usage policy (https://operations.osmfoundation.org/policies/nominatim/) --
  // the browser version can't reliably set a custom User-Agent, so this pipeline is the one place that can.
  const url = `${NOMINATIM_SEARCH_URL}?format=json&limit=1&q=${encodeURIComponent(q)}`
  try {
    const res = await fetchWithRetry(url, 'Nominatim geocoder', { headers: { 'User-Agent': 'ScoutSNF-roster-pipeline/1.0 (github.com/ScoutSNF/scoutsnf.github.io)' } }, { attempts: 2 })
